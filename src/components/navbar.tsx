    ...
    <nav className={`flex w-full justify-center fixed  ${scrollY  <= 50 && !isMenuOpen ? "bg-transparent text-[#fff] py-3" : "bg-[#fff] text-[#000] border-b border-[#000]"} z-10`}>
      <div className="z-50 w-full px-5 lg:px-0 lg:w-[1000px] xl:w-[1200px] flex justify-between py-5 items-center font-[700] text-lg ">
        ...
        <div className="hidden lg:flex gap-5">
          <div className=" flex gap-5">
            {siteConfig.getHeaderLinks().map((item, index) => <Link key={index} href={item.href} className={`${router.asPath === item.href ? "text-primary" : "text-inherit"}`} locale={t}>{item[t]}</Link>)}
          </div>
      ...
      {isMenuOpen && <div className="lg:hidden grid place-content-center z-40 fixed top-0 left-0 h-screen w-screen bg-[#fff] text-center">
        <div className="h-fit gap-[30px] flex flex-col justify-between text-[23px]">
          {siteConfig.getHeaderLinks().map(item => <Link onClick={() => setIsMenuOpen(false)} key={item.en} href={item.href} className={`${router.asPath === item.href ? "text-primary" : "text-inherit"}`}>{item[t]}</Link>)}
        </div>
